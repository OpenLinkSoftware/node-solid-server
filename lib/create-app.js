...
const corsSettings = cors({
  ...
  exposedHeaders: 'Authorization, User, Location, Link, Vary, Last-Modified, ETag, Accept-Patch, Accept-Post, Updates-Via, Allow, WAC-Allow, Content-Length, WWW-Authenticate, On-Behalf-Of, webid-tls',
  credentials: true,
  maxAge: 1728000,
  origin: true,
...
function createApp (argv = {}) {
  ...
  routeResolvedFile(app, '/common/js/', 'bootstrap/dist/js/bootstrap.min.js')
...
function initAppLocals (app, argv, ldp) {
  ...
  app.locals.solidVersion = argv.version
  app.locals.certDays = argv.certDays || 60
...
function initHeaders (app) {
  ...
  app.use((req, res, next) => {
    // Set X-Powered-By
    res.set('X-Powered-By', 'OpenLink Solid Server/'+ version)
