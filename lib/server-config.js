...
async function ensureWelcomePage (argv) {
  ...
  let defaultIcon = path.join(templates.server, 'favicon.ico')
  let existingIcon = path.join(serverRootDir, 'favicon.ico')
  let defaultIconAcl = path.join(templates.server, 'favicon.ico.acl')
  let existingIconAcl = path.join(serverRootDir, 'favicon.ico.acl')

  if (!fs.existsSync(existingIcon)) {
    fs.copySync(defaultIcon, existingIcon)
    fs.copySync(defaultIconAcl, existingIconAcl)
  }
}
