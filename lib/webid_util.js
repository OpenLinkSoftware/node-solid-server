...
WebIdUtils.prototype = {
  ...
  get_pimStore_for_WebID: function(webId) {
    var me = this;
    var profile_tmp_graph_id = null;
    var base = null;
    var webId_pimStore = null;
	...
	return new Promise(function(resolve, reject) {
	  rdfstore.create(function(err, rdf_store) {
	    ...
	    switch(profile.content_type)
	    {
	      ...
	      default:
	        throw new Error("WebIdUtils#get_pimStore_for_WebId: Delegate's profile: Unexpected WebID profile content type");
	    }
	  });
	});
	...
	return me.find_pimStore_in_webId_profile(
	  me.store,
	  profile_tmp_graph_id, 
	  webId);

      }).then(function (pim_store) {
        webId_pimStore = pim_store;
        me.clear_graph(me.store, profile_tmp_graph_id);
	resolve(pim_store);
      }).catch(function (e) {
	debug('WebIdUtils#get_pimStore_for_WebId: ', e)
	reject(e);
      });
    });
  },
...
*****/
	...
	if (err) {
	  reject(new Error('Failed to fetch profile from ' + uri.href + ': ' + err))
	}

	if (res.statusCode !== 200) {
	  reject (new Error('Failed to retrieve WebID from ' + uri.href + ': HTTP status: ' + res.statusCode))
	}
  ...
  find_pimStore_in_webId_profile: function (store, graph, webId) 
  {
    return new Promise(function(resolve, reject) {
      var qry = `
	prefix cert: <http://www.w3.org/ns/auth/cert#> 
	...
	`;
      ...
      store.execute(qry, function(err, results) {
        if (err) {
          ...
          var pim_store = null;
	  if (results.length == 1) {
	    pim_store = results[0].pim_store.value;
	  }
	  resolve(pim_store);
        }
