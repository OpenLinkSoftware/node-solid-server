...
class TlsAuthenticator extends Authenticator {
  ...
  constructor (options) {
    ...
    this.webid_util = options.webid_util
  }
  ...
  static fromParams (req, options) {
    ...
    options.webid_util = req.app.locals.webid_util
    options.username = body.username
  ...
  validate () {
    ...
    return this.accountManager.accountExists(this.username)
      .then(exists => {
        if (!exists) {
          debug(`Account ${this.username} is not existed`)
          throw new Error(`Account ${this.username} is not existed`)
        }
  ...
  findValidUser () {
    return this.renegotiateTls()
      ...
      .then(webId_data => this.loadUser(webId_data))
   ...
   *
   * @return {Promise<{webId:string, pimStore:string}>} WebID data
   */
   ...
   *
   * @param certificate {X509Certificate}
   * @param callback {Function} Gets invoked with signature `callback(error, webId_data)`
   */
  verifyWebId (certificate, auth_callback) {
    debug('Verifying WebID URI')

    var me = this;
    var delegator = me.headers['on-behalf-of'] || me.headers['On-Behalf-Of'];

    if (!delegator)
      webid.verify(certificate, resolve_pimStore_callback);
    else
      webid.verify(certificate, webid_verification_callback);


    function resolve_pimStore_callback(err, webId)
    {
      if (err)
        auth_callback(err, {webId:webId, pimStore:null});
      else
        me.webid_util.get_pimStore_for_WebID(webId)
         .then(function (pim_store) {

          auth_callback(null, {webId:webId, pimStore:pim_store});
        }).catch(function (e) {
          auth_callback(null, {webId:webId, pimStore:null});
        });
    }
    ...
    {
      if (err)
        auth_callback(err, {webId:delegate, pimStore:null});
      else
        webid_verify_delegation(delegator, delegate, certificate);
    }


    function webid_verify_delegation (delegator, delegate, delegate_certificate) 
    {
      debug('#webid_verify_delegation: delegate_certificate: ');
      ...
      me.webid_util.verify_delegation(delegator, delegate, delegate_certificate)
       .then(function (result) {
        ...
        resolve_pimStore_callback(null, delegator);
      }).catch(function (e) {
        ...
        resolve_pimStore_callback(null, delegate);
      });
   ...
   *
   * @param webId_data {webId:string, pimStore:string}
   *
   * @return {UserAccount}
   */
  loadUser (webId_data) {
    const serverUri = this.accountManager.host.serverUri
    const webId = webId_data.webId;

    if (domainMatches(serverUri, webId_data.webId)) {
...
/****
      ...
      return this.accountManager.userAccountFrom({ webId, username: webId, externalWebId: true, pimStore: webId_data.pimStore })
****/
      let user = this.accountManager.userAccountFrom({username: this.username, webId, externalWebId: true, pimStore: webId_data.pimStore})
