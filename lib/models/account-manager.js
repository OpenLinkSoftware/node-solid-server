...
class AccountManager {
  ...
  addCertKeyToProfile (certificate, userAccount) {
    ...
    return this.getProfileGraphFor(userAccount)
      .then(profileGraph => {
        return this.addCertKeyToGraph(certificate, profileGraph, userAccount)
      })
      .then(profileGraph => {
   ...
   * @return {Graph}
   */
  addCertKeyToGraph (certificate, graph, userAccount) {
    ...
    if (uri) {
      let sparql = `DELETE DATA 
                  { <${uri}#me> <http://www.w3.org/ns/auth/cert#key> <${uri}#key-stub> .
                    <${uri}#key-stub>  a <http://www.w3.org/ns/auth/cert#RSAPublicKey> ;
                       <http://www.w3.org/ns/auth/cert#exponent> "65537"^^<http://www.w3.org/2001/XMLSchema#int> ;
                       <http://www.w3.org/ns/auth/cert#modulus> "00"^^<http://www.w3.org/2001/XMLSchema#hexBinary> .
                  }`
      let patch = rdf.sparqlUpdateParser(sparql, rdf.graph(), uri)
      graph.applyPatch(patch, rdf.sym(uri), (s) => {})
    }
  ...
  createAccountFor (userAccount) {
    const template = AccountTemplate.for(userAccount, {}, this)
