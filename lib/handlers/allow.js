...
function allow (mode, checkPermissionsForDirectory) {
  return async function allowHandler (req, res, next) {
    ...
    const userId = req.session.userId || req.session.tempWebid
    let userOrigins = [];

    if (userId) {
      const profile = await ldp.fetchGraph(userId)
      if (profile) {
        let origins = profile.each($rdf.sym(userId), nsACL('origin'), undefined)
    ...
    let trustedOrigins = [ldp.resourceMapper.resolveUrl(req.hostname)].concat(ldp.trustedOrigins, userOrigins)
    if (ldp.multiuser) {
