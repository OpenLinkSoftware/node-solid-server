...
const nsACL = $rdf.Namespace('http://www.w3.org/ns/auth/acl#')
...
function allow (mode) {
  return async function allowHandler (req, res, next) {
    ...
    const tempWebid = req.session.tempWebid
    const webId = userId || tempWebid
    let userOrigins = [];

    if (webId) {
      const profile = await ldp.fetchGraph(webId)
      if (profile) {
        let origins = profile.each($rdf.sym(webId), nsACL('origin'), undefined)
        origins.forEach(origin => {
          userOrigins.push(origin.uri)
        })
      }
    }
    ...
    req.acl = new ACL(rootUrl + reqPath, {
      ...
      trustedOrigins: [ldp.resourceMapper.resolveUrl(req.hostname)].concat(ldp.trustedOrigins, userOrigins)
    })

    const isAllowed = await req.acl.can(webId, mode)
    if (isAllowed) {
      return next()
    }
    const error = await req.acl.getError(webId, mode)
    debug(`${mode} access denied to ${webId || '(none)'}: ${error.status} - ${error.message}`)
    next(error)    
  }
}
