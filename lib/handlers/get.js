...
async function handler (req, res, next) {
  ...
  if (requestedType && requestedType.includes('text/html')) {
    ...
    if (useDataBrowser) {
      ...
      let path
      try {
        path = await fetchPref(ldp.resourceMapper, ldp.resourceMapper.resolveUrl(req.hostname))
      } catch (err) {
      }
...
async function fetchPref (mapper, rootUrl) {
  let prefUrl = rootUrl + '/settings/prefs.ttl'

  // Convert the URL into a filename
  let path
  ({ path } = await mapper.mapUrlToFile({ url: prefUrl }))
