  ...
  ldp.get(options, function (err, ret) {
    ...
    if (requestedType && requestedType.includes('text/html')) {
      ...
      if (useDataBrowser) {
        ...
        let dataBrowserPath = defaultDataBrowser
        if (ldp.dataBrowserPath !== 'default') {
          dataBrowserPath = ldp.dataBrowserPath.replace('{url}', baseUri)
        }

        fetchPref(ldp, utils.getBaseUri(req), (e, path) => {
          if (path) {
            if (path !== 'default') {
              dataBrowserPath = path.replace('{url}', baseUri)
            } else {
              dataBrowserPath = defaultDataBrowser
            }
          }

          if (dataBrowserPath.startsWith('http://') || dataBrowserPath.startsWith('https://')) {
            debug('   sending redirect: ' + dataBrowserPath)
            res.redirect(dataBrowserPath)
          } else {
...
function fetchPref (ldp, rootUrl, callback) {
  let prefUrl = rootUrl + '/settings/prefs.ttl'
  ...
  mapper.mapUrlToFile({url: prefUrl})
  ...
  .then(body => {
    ...
    $rdf.parse(body, graph, prefUrl, 'text/turtle')
    return graph
  })
  .then(graph => Promise.resolve()
    .then(() => {
      let dataBrowserPath = null
      if (graph) {
        let rc = graph.any(graph.sym(prefUrl), graph.sym('https://www.w3.org/ns/solid/terms#dataBrowser'), undefined)
        if (rc) {
          dataBrowserPath = rc.value
        }
      }
  ...
  .then(dataBrowserPath => callback(null, dataBrowserPath), callback)
}
