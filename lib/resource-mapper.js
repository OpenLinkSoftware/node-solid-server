...
class ResourceMapper {
  ...
  async uriToFilename ({ url, contentType, createIfNotExists, searchIndex = true }) {
    var fullPath = this._getFullPath(url)
    ...
    if (fileStats && fileStats.isDirectory()) {
      ...
      return { path: fullPath, contentType: null }
    } else {
      return await this.mapUrlToFile({ url, createIfNotExists, searchIndex })
    }
  }
  ...
  async mapFileToUrl ({ path, hostname }) {
    ...
    let prefix = this._rootPath + '/' + hostname
    const pathname = this._removeDollarExtension(path.substring(prefix.length))
