...
function handle_webid (is_auth, req, res, callback) {
  ...
  if (req.session.userId) {
    ...
    return callback({err:null, httpErr:null, cert: null});
  }
  ...
  if (!certificate) {
    ...
    return callback({err:err_msg, httpErr, cert: null});
  }
  ...
  else
  {
    ...
    return callback({err:err_msg, httpErr, cert: certificate});
  }
  ...
  function authentication_callback(err, uri) 
  {
    if (err) {
      ...
      return callback({err:msg, httpErr, cert: certificate});
    }
    req.session.userId = uri
    debug('Identified user: ' + req.session.userId)
    res.set('User', req.session.userId)

    return callback({err:null, httpErr:null, cert: certificate});
  }
...
function getCertificate (req) {
  const certificate = getCertificateViaTLS(req) || getCertificateViaHeader(req)
  return {err:err_msg, cert:certificate};
}
