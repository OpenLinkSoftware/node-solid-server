...
class LDP {
  ...
  async post (host, containerPath, stream, { container, slug, extension }) {
    ...
    const { url: putUrl, contentType } = await this.resourceMapper.mapFileToUrl(
      { path: this.resourceMapper.rootPath + '/' + host + resourcePath, hostname: host })
  ...
  async get (options, searchIndex = true) {
    ...
    try {
      ({ path: filename, contentType } = await this.resourceMapper.uriToFilename({ url: options, searchIndex }))
      stats = await this.stat(filename)
    } catch (err) {
      throw error(404, 'Can\'t find file requested: ' + options.path + err)
    }
